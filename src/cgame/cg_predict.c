...
static void CG_ClipMoveToEntities ( const vector3 *start, const vector3 *mins, const vector3 *maxs, const vector3 *end,
	...
	for ( i = 0 ; i < cg_numSolidEntities ; i++ ) {
		...
		if ( ent->solid == SOLID_BMODEL ) {
			...
			bmins.x = bmins.y = (float)-x;
			bmaxs.x = bmaxs.y = (float) x;
			bmins.z = (float)-zd;
			bmaxs.z = (float) zu;
...
// Generates cg.predictedPlayerState by interpolating between cg.snap->playerState and cg.nextFrame->player_state
...
static void CG_TouchItem( centity_t *cent ) {
	...
	if( cgs.gametype == GT_FLAGS ) {
		if (cg.predictedPlayerState.persistent[PERS_TEAM] == TEAM_RED &&
			item->giTag == PW_REDFLAG)
			return;
		if (cg.predictedPlayerState.persistent[PERS_TEAM] == TEAM_BLUE &&
