...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker) {
	...
	if (targ->client->pers.teamState.lasthurtcarrier &&
		...
		attacker->client->ps.persistent[PERS_DEFEND_COUNT]++;
...
int Team_TouchOurFlag( gentity_t *ent, gentity_t *other, int team ) {
	...
	other->client->ps.persistent[PERS_CAPTURES]++;
	...
	for (i = 0; i < sv_maxclients->integer; i++) {
		...
		if (player->client->sess.sessionTeam !=
			...
			if (player->client->pers.teamState.lastreturnedflag + 
				...
				player->client->ps.persistent[PERS_ASSIST_COUNT]++;
				// add the sprite over the player's head
...
gentity_t *SelectCTFSpawnPoint ( team_t team, int teamstate, vector3 *origin, vector3 *angles, qboolean isbot ) {
	gentity_t	*spot;
