...
static gentity_t *SpawnModelOnVictoryPad( gentity_t *pad, vec3_t offset, gentity_t *ent, int place ) {
	...
	if ( body->s.weapon == WP_NONE )
		body->s.weapon = WP_QUANTIZER;
	body->s.legsAnim = LEGS_IDLE;
	body->s.torsoAnim = weaponAnimations[body->s.weapon];// TORSO_STAND;
	body->s.event = 0;
...
static void CelebrateStop( gentity_t *player ) {
	player->s.torsoAnim = ( ( player->s.torsoAnim & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) | weaponAnimations[player->s.weapon];
}
...
static void PodiumPlacementThink( gentity_t *podium ) {
	...
	VectorMA( level.intermission_origin, gi.Cvar_VariableIntegerValue( "g_podiumDist" ), vec, origin );
	origin[2] -= gi.Cvar_VariableIntegerValue( "g_podiumDrop" );
...
static gentity_t *SpawnPodium( void ) {
	...
	gi.SV_LinkEntity ((sharedEntity_t *)podium);
