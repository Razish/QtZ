...
void	G_TouchTriggers( gentity_t *ent ) {
	...
	for ( i=0 ; i<num ; i++ ) {
		...
		if ( hit->s.eType == ET_ITEM ) {
			...
			if ( !trap->SV_EntityContact( &mins, &maxs, (sharedEntity_t *)hit, qfalse ) ) {
				continue;
			}
		}
		...
		{//Must be within half a unit to actually teleport
			...
			if ( !trap->SV_EntityContact( &newMins, &newMaxs, (sharedEntity_t *)hit, qfalse ) )
				continue;
		}
		//~QtZ
...
//	i = trap->aas->AAS_PointReachabilityAreaIndex( ent->client->ps.origin );
