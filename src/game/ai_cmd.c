...
int BotGetItemTeamGoal(char *goalname, bot_goal_t *goal) {
	...
	i = -1;
	do {
		i = trap->ai->BotGetLevelItemGoal(i, goalname, goal);
		if (i > 0) {
...
float BotGetTime(bot_match_t *match) {
	...
	if (match->subtype & ST_TIME) {
		//get the time string
		trap->ai->BotMatchVariable(match, TIME, timestring, MAX_MESSAGE_SIZE);
		//match it to find out if the time is in seconds or minutes
		if (trap->ai->BotFindMatch(timestring, &timematch, MTCONTEXT_TIME)) {
			if (timematch.type == MSG_FOREVER) {
				...
				trap->ai->BotMatchVariable(&timematch, TIME, timestring, MAX_MESSAGE_SIZE);
...
int NumPlayersOnSameTeam(bot_state_t *bs) {
	...
	num = 0;
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		trap->SV_GetConfigstring(CS_PLAYERS+i, buf, MAX_INFO_STRING);
		if (strlen(buf)) {
...
int BotGetPatrolWaypoints(bot_state_t *bs, bot_match_t *match) {
	...
	patrolflags = 0;
	//
	trap->ai->BotMatchVariable(match, KEYAREA, keyarea, MAX_MESSAGE_SIZE);
	//
	while(1) {
		if (!trap->ai->BotFindMatch(keyarea, &keyareamatch, MTCONTEXT_PATROLKEYAREA)) {
			trap->ea->EA_SayTeam(bs->client, "what do you say?");
			BotFreeWaypoints(newpatrolpoints);
			bs->patrolpoints = NULL;
			return qfalse;
		}
		trap->ai->BotMatchVariable(&keyareamatch, KEYAREA, keyarea, MAX_MESSAGE_SIZE);
		...
		if (keyareamatch.subtype & ST_BACK) {
			...
			trap->ai->BotMatchVariable(&keyareamatch, MORE, keyarea, MAX_MESSAGE_SIZE);
		}
		else {
			break;
		}
	...
	if (!newpatrolpoints || !newpatrolpoints->next) {
		trap->ea->EA_SayTeam(bs->client, "I need more key points to patrol\n");
		BotFreeWaypoints(newpatrolpoints);
		newpatrolpoints = NULL;
		return qfalse;
	}
...
int BotAddressedToBot(bot_state_t *bs, bot_match_t *match) {
	...
	if (match->subtype & ST_ADDRESSED) {
		trap->ai->BotMatchVariable(match, ADDRESSEE, addressedto, sizeof(addressedto));
		//the name of this bot
		ClientName(bs->client, botname, 128);
		//
		while(trap->ai->BotFindMatch(addressedto, &addresseematch, MTCONTEXT_ADDRESSEE)) {
			if (addresseematch.type == MSG_EVERYONE) {
				return qtrue;
			}
			else if (addresseematch.type == MSG_MULTIPLENAMES) {
				trap->ai->BotMatchVariable(&addresseematch, TEAMMATE, name, sizeof(name));
				if (strlen(name)) {
					if (stristr(botname, name)) return qtrue;
					if (stristr(bs->subteam, name)) return qtrue;
				}
				trap->ai->BotMatchVariable(&addresseematch, MORE, addressedto, MAX_MESSAGE_SIZE);
			}
			else {
				trap->ai->BotMatchVariable(&addresseematch, TEAMMATE, name, MAX_MESSAGE_SIZE);
				if (strlen(name)) {
					if (stristr(botname, name)) return qtrue;
					if (stristr(bs->subteam, name)) return qtrue;
				}
...
void BotMatch_HelpAccompany(bot_state_t *bs, bot_match_t *match) {
	...
	if (trap->ai->BotFindMatch(teammate, &teammatematch, MTCONTEXT_TEAMMATE) &&
	...
	if (bs->teamgoal.entitynum < 0) {
		...
		if (match->subtype & ST_NEARITEM) {
			...
			trap->ai->BotMatchVariable(match, ITEM, itemname, sizeof(itemname));
			//
			if (!BotGetMessageTeamGoal(bs, itemname, &bs->teamgoal)) {
				//BotAI_BotInitialChat(bs, "cannotfind", itemname, NULL);
				//trap->ai->BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
				return;
			}
		}
	}
	...
	if (bs->teamgoal.entitynum < 0) {
		...
		client = ClientFromName(netname);
		trap->ai->BotEnterChat(bs->cs, client, CHAT_TEAM);
		return;
	}
	//the team mate
	bs->teammate = client;
...
void BotMatch_DefendKeyArea(bot_state_t *bs, bot_match_t *match) {
	...
	if (!BotAddressedToBot(bs, match)) return;
	//get the match variable
	trap->ai->BotMatchVariable(match, KEYAREA, itemname, sizeof(itemname));
	//
	if (!BotGetMessageTeamGoal(bs, itemname, &bs->teamgoal)) {
		//BotAI_BotInitialChat(bs, "cannotfind", itemname, NULL);
		//trap->ai->BotEnterChat(bs->cs, bs->client, CHAT_TEAM);
...
void BotMatch_TaskPreference(bot_state_t *bs, bot_match_t *match) {
	...
	trap->ai->BotEnterChat(bs->cs, teammate, CHAT_TELL);
...
void BotMatch_JoinSubteam(bot_state_t *bs, bot_match_t *match) {
	...
	trap->ai->BotMatchVariable(match, TEAMNAME, teammate, sizeof(teammate));
...
void BotMatch_CheckPoint(bot_state_t *bs, bot_match_t *match) {
	...
	if (!TeamPlayIsOn()) return;
	//
	trap->ai->BotMatchVariable(match, POSITION, buf, MAX_MESSAGE_SIZE);
	...
	//
	trap->ai->BotMatchVariable(match, NAME, buf, MAX_MESSAGE_SIZE);
...
void BotMatch_FormationSpace(bot_state_t *bs, bot_match_t *match) {
	...
	if (!BotAddressedToBot(bs, match)) return;
	//
	trap->ai->BotMatchVariable(match, NUMBER, buf, MAX_MESSAGE_SIZE);
...
void BotMatch_Suicide(bot_state_t *bs, bot_match_t *match) {
	...
	if (!BotAddressedToBot(bs, match)) return;
	//
	trap->ea->EA_Command(bs->client, "kill");
	//
	trap->ai->BotMatchVariable(match, NETNAME, netname, sizeof(netname));
	client = ClientFromName(netname);
	//
...
float BotNearestVisibleItem(bot_state_t *bs, char *itemname, bot_goal_t *goal) {
	...
	i = -1;
	do {
		i = trap->ai->BotGetLevelItemGoal(i, itemname, &tmpgoal);
		trap->ai->BotGoalName(tmpgoal.number, name, sizeof(name));
...
void BotMatch_Kill(bot_state_t *bs, bot_match_t *match) {
	...
	trap->ai->BotMatchVariable(match, ENEMY, enemy, sizeof(enemy));
	//
...
int BotMatchMessage(bot_state_t *bs, char *message) {
	...
	if (!trap->ai->BotFindMatch(message, &match, MTCONTEXT_MISC
	...
	switch(match.type)
	{
		...
		{
			trap->ea->EA_SayTeam(bs->client, "the part of my brain to create formations has been damaged");
			break;
		}
