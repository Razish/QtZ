...
void RE_SetColor( const vector4 *rgba ) {
	...
	if ( !(cmd = (setColorCommand_t *)R_GetCommandBuffer( sizeof( *cmd ) )) )
		return;

	cmd->commandId = RC_SET_COLOR;
	if ( !rgba ) {
		static vector4 colorWhite = { 1, 1, 1, 1 };
		rgba = &colorWhite;
	}
...
void RE_PostProcess( void ) {
    ...
    //Xycaleth: After post processing, we switch back to the regular back buffer, to draw the hud on
    cmd2 = (drawBufferCommand_t *)R_GetCommandBuffer (sizeof (*cmd2));
    if ( !cmd2 )
        return;

    cmd2->commandId = RC_DRAW_BUFFER;
...
void RE_RotatedPic( float x, float y, float w, float h,
	...
	cmd->x += cmd->w;
	cmd->y += cmd->h;
	cmd->w = sqrtf( ( cmd->w * cmd->w ) + ( cmd->h * cmd->h ) );
	cmd->h = cmd->w;
...
//QTZTODO: RC_RENDERTOTEXTURE - distortion
//QTZTODO: RC_FINISH
