...
static qboolean IQM_CheckRange( iqmHeader_t *header, unsigned int offset, int count, int size ) {
...
static void JointToMatrix( vector4 *rot, vector3 *scale, vector3 *trans, float *mat ) {
	float xx = 2.0f * rot->x * rot->x;
	float yy = 2.0f * rot->y * rot->y;
	float zz = 2.0f * rot->z * rot->z;
	float xy = 2.0f * rot->x * rot->y;
	float xz = 2.0f * rot->x * rot->z;
	float yz = 2.0f * rot->y * rot->z;
	float wx = 2.0f * rot->w * rot->x;
	float wy = 2.0f * rot->w * rot->y;
	float wz = 2.0f * rot->w * rot->z;

	mat[ 0] = scale->x * (1.0f - (yy + zz));
	mat[ 1] = scale->x * (xy - wz);
	mat[ 2] = scale->x * (xz + wy);
	mat[ 3] = trans->x;
	mat[ 4] = scale->y * (xy + wz);
	mat[ 5] = scale->y * (1.0f - (xx + zz));
	mat[ 6] = scale->y * (yz - wx);
	mat[ 7] = trans->y;
	mat[ 8] = scale->z * (xz - wy);
	mat[ 9] = scale->z * (yz + wx);
...
static void Matrix34Invert( float *inMat, float *outMat )
{
	...
	v = (vector3 *)(outMat + 0); invSqrLen = 1.0f / DotProduct(v, v); VectorScale(v, invSqrLen, v);
	v = (vector3 *)(outMat + 4); invSqrLen = 1.0f / DotProduct(v, v); VectorScale(v, invSqrLen, v);
	v = (vector3 *)(outMat + 8); invSqrLen = 1.0f / DotProduct(v, v); VectorScale(v, invSqrLen, v);

	trans.x = inMat[ 3];
	trans.y = inMat[ 7];
	trans.z = inMat[11];

	outMat[ 3] = -DotProduct((vector3 *)&outMat + 0, &trans);
	outMat[ 7] = -DotProduct((vector3 *)&outMat + 4, &trans);
...
qboolean R_LoadIQM( model_t *mod, void *buffer, unsigned int filesize, const char *mod_name ) {
	...
	for( i = 0; i < header->num_joints; i++, joint++ ) {
		LL( joint->name );
		LL( joint->parent );
		LL( joint->translate.x );
		LL( joint->translate.y );
		LL( joint->translate.z );
		LL( joint->rotate.x );
		LL( joint->rotate.y );
		LL( joint->rotate.z );
		LL( joint->rotate.w );
	...
	for( i = 0; i < header->num_joints; i++, joint++ ) {
		...
		else
		{
			...
			mat += 12;
			memcpy( mat, invBaseFrame, sizeof(invBaseFrame) );
			mat += 12;
		}
	}
	...
	for( i = 0; i < header->num_frames; i++ ) {
		...
		for( j = 0; j < header->num_poses; j++, pose++ ) {
			...
			translate.x = pose->channeloffset[0];
			if ( pose->mask & 0x001 )	translate.x += *framedata++ * pose->channelscale[0];
			translate.y = pose->channeloffset[1];
			if ( pose->mask & 0x002 )	translate.y += *framedata++ * pose->channelscale[1];
			translate.z = pose->channeloffset[2];
			if ( pose->mask & 0x004 )	translate.z += *framedata++ * pose->channelscale[2];

			rotate.x = pose->channeloffset[3];
			if ( pose->mask & 0x008 )	rotate.x += *framedata++ * pose->channelscale[3];
			rotate.y = pose->channeloffset[4];
			if ( pose->mask & 0x010 )	rotate.y += *framedata++ * pose->channelscale[4];
			rotate.z = pose->channeloffset[5];
			if ( pose->mask & 0x020 )	rotate.z += *framedata++ * pose->channelscale[5];
			rotate.w = pose->channeloffset[6];
			if ( pose->mask & 0x040 )	rotate.w += *framedata++ * pose->channelscale[6];

			scale.x = pose->channeloffset[7];
			if ( pose->mask & 0x080 )	scale.x += *framedata++ * pose->channelscale[7];
			scale.y = pose->channeloffset[8];
			if ( pose->mask & 0x100 )	scale.y += *framedata++ * pose->channelscale[8];
			scale.z = pose->channeloffset[9];
			if ( pose->mask & 0x200 )	scale.z += *framedata++ * pose->channelscale[9];
...
static int R_CullIQM( iqmData_t *data, trRefEntity_t *ent ) {
	...
	for (i = 0 ; i < 3 ; i++) {
		bounds[0].data[i] = oldBounds[i+0] < newBounds[i+0] ? oldBounds[i+0] : newBounds[i+0];
		bounds[1].data[i] = oldBounds[i+3] > newBounds[i+3] ? oldBounds[i+3] : newBounds[i+3];
	}
...
int R_ComputeIQMFogNum( iqmData_t *data, trRefEntity_t *ent ) {
	...
	fog_t			*fog;
	const number	*bounds;
	const number	defaultBounds[6] = { -8, -8, -8, 8, 8, 8 };
	vector3			diag, center;
	vector3			localOrigin;
	number			radius;
	...
	VectorSubtract( (vector3 *)(bounds+3), (vector3 *)bounds, &diag );
	VectorMA( (vector3 *)bounds, 0.5f, &diag, &center );
	...
	for ( i = 1 ; i < tr.world->numfogs ; i++ ) {
		fog = &tr.world->fogs[i];
		for ( j = 0 ; j < 3 ; j++ ) {
			if ( localOrigin.data[j] - radius >= fog->bounds[1].data[j] )	break;
			if ( localOrigin.data[j] + radius <= fog->bounds[0].data[j] )	break;
		}
		if ( j == 3 ) {
			return i;
		}
...
void RB_IQMSurfaceAnim( surfaceType_t *surface ) {
	...
	for( i = 0; i < surf->num_vertexes;
	     i++, outXYZ++, outNormal++, outTexCoord += 2, outColor++ ) {
		int	j, k;
		float	vtxMat[12];
		float	nrmMat[9];
		...
		// four blend weights
		for( k = 0; k < 12; k++ )
			vtxMat[k] = data->blendWeights[4*vtx] * jointMats[12*data->blendIndexes[4*vtx] + k];
		for( j = 1; j < 4; j++ ) {
			if( data->blendWeights[4*vtx + j] <= 0 )
				break;
			for( k = 0; k < 12; k++ )
				vtxMat[k] += data->blendWeights[4*vtx + j] * jointMats[12*data->blendIndexes[4*vtx + j] + k];
		}
		for( k = 0; k < 12; k++ )
		...
		outTexCoord[0].x = data->texcoords[2*vtx + 0];
		outTexCoord[0].y = data->texcoords[2*vtx + 1];
		outTexCoord[1].x = outTexCoord[0].x;
		outTexCoord[1].y = outTexCoord[0].y;
...
int R_IQMLerpTag( orientation_t *tag, iqmData_t *data, int startFrame, int endFrame, float frac, const char *tagName ) {
	...
	tag->axis[0].x	= jointMats[12 * joint +  0];
	tag->axis[1].x	= jointMats[12 * joint +  1];
	tag->axis[2].x	= jointMats[12 * joint +  2];
	tag->origin.x	= jointMats[12 * joint +  3];
	tag->axis[0].y	= jointMats[12 * joint +  4];
	tag->axis[1].y	= jointMats[12 * joint +  5];
	tag->axis[2].y	= jointMats[12 * joint +  6];
	tag->origin.y	= jointMats[12 * joint +  7];
	tag->axis[0].z	= jointMats[12 * joint +  8];
	tag->axis[1].z	= jointMats[12 * joint +  9];
	tag->axis[2].z	= jointMats[12 * joint + 10];
	tag->origin.z	= jointMats[12 * joint + 11];
