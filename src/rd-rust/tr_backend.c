...
void RB_BeginDrawingView (void) {
	...
	{
		framebuffer_t *lastFBO = (framebuffer_t *)R_EXT_GetCurrentFramebuffer();
	...
	if ( backEnd.viewParms.isPortal ) {
		...
		plane[0] = backEnd.viewParms.portalPlane.normal.x;
		plane[1] = backEnd.viewParms.portalPlane.normal.y;
		plane[2] = backEnd.viewParms.portalPlane.normal.z;
		plane[3] = backEnd.viewParms.portalPlane.dist;

		plane2[0] = DotProduct (&backEnd.viewParms.or.axis[0], (vector3 *)&plane);
		plane2[1] = DotProduct (&backEnd.viewParms.or.axis[1], (vector3 *)&plane);
		plane2[2] = DotProduct (&backEnd.viewParms.or.axis[2], (vector3 *)&plane);
		plane2[3] = DotProduct ((vector3 *)&plane, &backEnd.viewParms.or.origin) - plane[3];
...
const void	*RB_SetColor( const void *data ) {
	...
	backEnd.color2D[0] = (byte)(cmd->color.r * 255);
	backEnd.color2D[1] = (byte)(cmd->color.g * 255);
	backEnd.color2D[2] = (byte)(cmd->color.b * 255);
	backEnd.color2D[3] = (byte)(cmd->color.a * 255);
...
const void	*RB_DrawBuffer( const void *data ) {
	...
	//QtZ: post processing
	// If we're rendering to a color attachment, we need to bind the scene FBO first.
	if ( cmd->buffer >= GL_COLOR_ATTACHMENT0_EXT && cmd->buffer <= GL_COLOR_ATTACHMENT15_EXT )
		R_EXT_BindFramebuffer( tr.framebuffers.scene );
	else
		R_EXT_BindDefaultFramebuffer();
	//~QtZ
