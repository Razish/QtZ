...
void CheckGLErrors( const char* filename, int line ) {
	...
	if ( error != GL_NO_ERROR )
	{
		...
		case GL_INVALID_ENUM:
			Com_Printf (S_COLOR_RED "GL_INVALID_ENUM in file %s:%d.\n", filename, line);
			break;
		case GL_INVALID_VALUE:
			Com_Printf (S_COLOR_RED "GL_INVALID_VALUE in file %s:%d.\n", filename, line);
			break;
		case GL_INVALID_OPERATION:
			Com_Printf (S_COLOR_RED "GL_INVALID_OPERATION in file %s:%d.\n", filename, line);
			break;
		case GL_STACK_OVERFLOW:
			Com_Printf (S_COLOR_RED "GL_STACK_OVERFLOW in file %s:%d.\n", filename, line);
			break;
		case GL_STACK_UNDERFLOW:
			Com_Printf (S_COLOR_RED "GL_STACK_UNDERFLOW in file %s:%d.\n", filename, line);
			break;
		case GL_OUT_OF_MEMORY:
			Com_Printf (S_COLOR_RED "GL_OUT_OF_MEMORY in file %s:%d.\n", filename, line);
			break;
		default:
			Com_Printf (S_COLOR_RED "Error code 0x%X on line %d.\n", error, line);
			break;
		}
...
qboolean R_EXT_FramebufferInit( void ) {
	...
	{
		Com_Printf( S_COLOR_RED "Framebuffer extension NOT loaded.\nRequired OpenGL extensions not available.\n" );
		return qfalse;
	}
...
//  memset( renderbuffers,	0,	sizeof( renderbuffers )	);
	...
	Com_Printf( "Framebuffer extension loaded\n" );
...
void R_EXT_CheckFramebuffer( const framebuffer_t *framebuffer ) {
	...
	case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT_EXT:
		Com_Printf( "One or more framebuffer attachment points are not complete.\n" );
		break;

	case GL_FRAMEBUFFER_INCOMPLETE_DIMENSIONS_EXT:
		Com_Printf( "One or more attached images have different dimensions.\n" );
		break;

	case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER_EXT:
		Com_Printf( "Invalid framebuffer attachment object type used.\n" );
		break;

	case GL_FRAMEBUFFER_INCOMPLETE_FORMATS_EXT:
		Com_Printf( "More than one internal format was used in the color attachments.\n" );
		break;

	case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER_EXT:
		Com_Printf( "Missing a read buffer.\n" );
		break;

	case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT_EXT:
		Com_Printf( "No images were attached to the framebuffer.\n" );
		break;
	...
	if ( status != GL_FRAMEBUFFER_COMPLETE_EXT )
		Com_Printf( "Creation of framebuffer %d could not be completed.\n", framebuffer->id );
...
texture_t *R_EXT_CreateTexture( unsigned int width, unsigned int height, internalFormat_t internalFormat, unsigned int minFilter, unsigned int magFilter ) {
	...
	{
		Com_Printf ("Exceeded maximum number of textures.\n");
		return NULL;
	}
	...
	if ( textureId == 0 )
	{
		Com_Printf ("Failed to create texture with internal ID %d.\n", numTextures);
		return NULL;
	}
	...
	//BADNESS AHEAD, so we can use GL_Bind and support Q3's built-in mutlitexture unit code for lightmaps and such
	texture->q3image = R_CreateBlankImage( va( "*SPECIAL%03d", numTextures ), width, height, GL_CLAMP_TO_EDGE );
	texture->q3image->texnum = textureId;
...
//		filterMode = GL_NEAREST;
	...
	if ( textureFilterAnisotropic )
		qglTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, (GLfloat)Q_clamp( 1.0f, r_ext_max_anisotropy->value, glConfig.maxTextureFilterAnisotropy ) );

	glTexParameteri( glTexTarget, GL_TEXTURE_MIN_FILTER, filterMode );
	glTexParameteri( glTexTarget, GL_TEXTURE_MAG_FILTER, filterMode );
	...
	//BADNESS AHEAD, so we can use GL_Bind and support Q3's built-in mutlitexture unit code for lightmaps and such
	texture->q3image = R_CreateBlankImage( va( "*BLANK%03d", numTextures ), width, height, GL_CLAMP_TO_EDGE );
	texture->q3image->texnum = textureId;
	texture->q3image->width = width;
	texture->q3image->height = height;
