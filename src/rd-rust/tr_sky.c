		...
		for ( t = 0; t <= SKY_SUBDIVISIONS; t++ )
		{
			for ( s = 0; s <= SKY_SUBDIVISIONS; s++ )
			{
				...
				p = ( 1.0f / ( 2 * DotProduct( &skyVec, &skyVec ) ) ) *
					...
					SQR( skyVec.x ) * SQR( heightCloud ) + 2 * SQR( skyVec.y ) * radiusWorld * heightCloud +
					SQR( skyVec.y ) * SQR( heightCloud ) + 2 * SQR( skyVec.z ) * radiusWorld * heightCloud +
