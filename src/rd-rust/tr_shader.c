...
static qboolean ParseVector( const char **text, int count, vector3 *v ) {
	char	*token;
	int		i;
...
static qboolean ParseStage( shaderStage_t *stage, const char **text ) {
	...
	while ( 1 )
	{
		...
		//QtZ: Glow buffer
		// surface will be rendered into off-screen buffer and blended during post-processing
		// if post-processing or glow is disabled, the surface will not be rendered at all.
		...
		else if ( !Q_stricmp( token, "rgbGen" ) )
		{
			...
			else if ( !Q_stricmp( token, "const" ) )
			{
				...
				stage->constantColor[0] = (byte)(255 * color.r);
				stage->constantColor[1] = (byte)(255 * color.g);
				stage->constantColor[2] = (byte)(255 * color.b);
		...
		else if ( !Q_stricmp( token, "alphaGen" ) )
		{
			...
			{
				token = COM_ParseExt( text, qfalse );
				stage->constantColor[3] = (byte)(255 * (float)atof( token ));
			...
			else if ( !Q_stricmp( token, "portal" ) )
			{
				...
				{
					shader.portalRange = (float)atof( token );
				}
			}
			else
			{
...
static void ParseDeform( const char **text ) {
	...
	if ( !Q_stricmp( token, "move" ) ) {
		...
		for ( i = 0 ; i < 3 ; i++ ) {
			...
			ds->moveVector.data[i] = (float)atof( token );
		}
...
static void ParseSkyParms( const char **text ) {
	...
	shader.sky.cloudHeight = (float)atof( token );
...
static qboolean ParseShader( const char **text ) {
	...
	while ( 1 )
	{
		...
		// sun parms
			...
			token = COM_ParseExt( text, qfalse );
			tr.sunLight.r = (float)atof( token );
			token = COM_ParseExt( text, qfalse );
			tr.sunLight.g = (float)atof( token );
			token = COM_ParseExt( text, qfalse );
			tr.sunLight.b = (float)atof( token );
			...
			tr.sunDirection.x = cosf( a ) * cosf( b );
			tr.sunDirection.y = sinf( a ) * cosf( b );
			tr.sunDirection.z = sinf( b );
		}
			...
			token = COM_ParseExt( text, qfalse );
			if (token[0]) {
				shader.clampTime = (float)atof(token);
			}
		}
		...
		else if ( !Q_stricmp( token, "fogParms" ) ) 
		{
			...
			shader.fogParms.depthForOpaque = (float)atof( token );
...
static shader_t *GeneratePermanentShader( void ) {
	...
	newShader = ri->Hunk_Alloc( sizeof( shader_t ), PREF_LOW );
	...
	*newShader = shader;
	...
	for ( i = 0 ; i < newShader->numUnfoggedPasses ; i++ ) {
		...
		newShader->stages[i] = ri->Hunk_Alloc( sizeof( stages[i] ), PREF_LOW );
		*newShader->stages[i] = stages[i];

		for ( b = 0 ; b < NUM_TEXTURE_BUNDLES ; b++ ) {
			size = newShader->stages[i]->bundle[b].numTexMods * sizeof( texModInfo_t );
			newShader->stages[i]->bundle[b].texMods = ri->Hunk_Alloc( size, PREF_LOW );
...
static void ScanAndLoadShaderFiles( void ) {
	...
	shaderFiles = ri->FS_ListFiles( SHADER_PATH, ".shader", &numShaderFiles );
	...
	for ( i = 0; i < numShaderFiles; i++ )
	{
		...
		Com_sprintf( filename, sizeof( filename ), SHADER_PATH"/%s", shaderFiles[i] );
	...
	s_shaderText = ri->Hunk_Alloc( sum + numShaderFiles*2, PREF_LOW );
	s_shaderText[ 0 ] = '\0';
	...
	hashMem = ri->Hunk_Alloc( size * sizeof(char *), PREF_LOW );
