...
void RE_AddPolyToScene( qhandle_t hShader, int numVerts, const polyVert_t *verts, int numPolys ) {
	...
	if ( !hShader ) {
		//QtZ RE_AddPolyToScene warnings should be for developers only
      ...
      */
		...
		// see if it is in a fog volume
			...
			for ( fogIndex = 1 ; fogIndex < tr.world->numfogs ; fogIndex++ ) {
				...
				if ( bounds[1].x >= fog->bounds[0].x &&
					 bounds[1].y >= fog->bounds[0].y &&
					 bounds[1].z >= fog->bounds[0].z &&
					 bounds[0].x <= fog->bounds[1].x &&
					 bounds[0].y <= fog->bounds[1].y &&
					 bounds[0].z <= fog->bounds[1].z ) {
					break;
				}
			}
...
void RE_AddRefEntityToScene( const refEntity_t *ent ) {
	...
	if ( Q_isnan(ent->origin.x) || Q_isnan(ent->origin.y) || Q_isnan(ent->origin.z) ) {
...
void RE_RenderScene( const refdef_t *fd ) {
	...
	//QtZ: Post processing
	if ( r_postprocess_enable->integer && (fd->rdflags & RDF_POSTPROCESS) )
	{
		drawBufferCommand_t *cmd = (drawBufferCommand_t *)R_GetCommandBuffer (sizeof (*cmd));
		cmd->commandId = RC_DRAW_BUFFER;
		cmd->buffer = (int)GL_COLOR_ATTACHMENT0_EXT;
	}
	//~QtZ
