...
static qboolean R_LoadMD3 (model_t *mod, int lod, void *buffer, const char *mod_name ) {
    ...
    for ( i = 0 ; i < mod->md3[lod]->numTags * mod->md3[lod]->numFrames ; i++, tag++) {
        for ( j = 0 ; j < 3 ; j++ ) {
			tag->origin.data[j] = LittleFloat( tag->origin.data[j] );
			tag->axis[0].data[j] = LittleFloat( tag->axis[0].data[j] );
			tag->axis[1].data[j] = LittleFloat( tag->axis[1].data[j] );
			tag->axis[2].data[j] = LittleFloat( tag->axis[2].data[j] );
        }
	}
...
static qboolean R_LoadMD4( model_t *mod, void *buffer, const char *mod_name ) {
	...
	for ( lodindex = 0 ; lodindex < md4->numLODs ; lodindex++ ) {
		...
		for ( i = 0 ; i < lod->numSurfaces ; i++) {
			...
			for ( j = 0 ; j < surf->numVerts ; j++ ) {
				v->normal.x = LittleFloat( v->normal.x );
				v->normal.y = LittleFloat( v->normal.y );
				v->normal.z = LittleFloat( v->normal.z );

				v->texCoords.x = LittleFloat( v->texCoords.x );
				v->texCoords.y = LittleFloat( v->texCoords.y );
				...
				for ( k = 0 ; k < v->numWeights ; k++ ) {
				   ...
				   v->weights[k].offset.x = LittleFloat( v->weights[k].offset.x );
				   v->weights[k].offset.y = LittleFloat( v->weights[k].offset.y );
				   v->weights[k].offset.z = LittleFloat( v->weights[k].offset.z );
				}
				// FIXME
				// This makes TFC's skeletons work.  Shouldn't be necessary anymore, but left
				// in for reference.
...
void R_ModelBounds( qhandle_t handle, vector3 *mins, vector3 *maxs ) {
	...
	if(model->type == MOD_BRUSH) {
		...
		if(iqmData->bounds)
		{
			VectorCopy((vector3 *)(iqmData->bounds), mins);
			VectorCopy((vector3 *)(iqmData->bounds + 3), maxs);
			return;
		}
	}
