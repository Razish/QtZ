...
const void *RB_TakeVideoFrameCmd( const void *data ) {
	...
	size_t						padwidth, avipadwidth, padlen, avipadlen;
	GLint						packAlign;
...
void R_Register( void )  {
	...
	r_znear								= ri->Cvar_Get( "r_znear",							"2",								CVAR_CHEAT,					NULL, NULL );
...
//	Swap_Init();
	...
	ptr = (byte *)ri->Hunk_Alloc( sizeof( *backEndData ) + sizeof(srfPoly_t) * max_polys + sizeof(polyVert_t) * max_polyverts, PREF_LOW);
