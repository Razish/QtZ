...
#define	SIDE_BACK	1
#define	SIDE_ON		2
static void R_ChopPolyBehindPlane( int numInPoints, vector3 inPoints[MAX_VERTS_ON_POLY], int *numOutPoints, vector3 outPoints[MAX_VERTS_ON_POLY], vector3 *normal, number dist, number epsilon ) {
	...
	*numOutPoints = 0;
	...
	if ( !counts[1] ) {
		*numOutPoints = numInPoints;
		memcpy( outPoints, inPoints, numInPoints * sizeof(vector3) );
		return;
	}
	...
	for ( i = 0 ; i < numInPoints ; i++ ) {
		...
		for (j=0 ; j<3 ; j++) {
			clip->data[j] = p1->data[j] + dot * ( p2->data[j] - p1->data[j] );
		}
...
void R_AddMarkFragments( int numClipPoints, vector3 clipPoints[2][MAX_VERTS_ON_POLY], int numPlanes, vector3 *normals, float *dists, int maxPoints, vector3 *pointBuffer,
	...
	*/
	...
	memcpy( pointBuffer + (*returnedPoints) * 3, clipPoints[pingPong], numClipPoints * sizeof(vector3) );
