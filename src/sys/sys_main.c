...
static const char *Sys_PIDFileName( void ) {
...
qboolean Sys_WritePIDFile( void ) {
	...
	if( ( f = fopen( pidFile, "r" ) ) != NULL )
	{
		...
		{
			pid = atoi( pidBuffer );
			if( !Sys_PIDIsRunning( (unsigned int)pid ) )
				stale = qtrue;
		}
		else
			stale = qtrue;
...
#ifdef DEDICATED
			fprintf( stdout, QTZ_VERSION " dedicated server (%s)\n", date );
#else
			fprintf( stdout, QTZ_VERSION " client (%s)\n", date );
#endif
			Sys_Exit( 0 );
		}
	}
