...
int AAS_BestReachableFromJumpPadArea(vector3 *origin, vector3 *mins, vector3 *maxs) {
	...
	vector3 absmins, absmaxs, velocity;
...
#else
	bot_visualizejumppads = (int)LibVarValue("bot_visualizejumppads", "0");
#endif
...
int AAS_Reachability_Swim(int area1num, int area2num) {
	...
	for (i = 0; i < 3; i++)
	{
		if (area1->mins.data[i] > area2->maxs.data[i] + 10) return qfalse;
		if (area1->maxs.data[i] < area2->mins.data[i] - 10) return qfalse;
	}
...
int AAS_Reachability_EqualFloorHeight(int area1num, int area2num) {
	...
	if (area2->mins.z > area1->maxs.z) return qfalse;
	//
	...
	if (foundreach)
	{
		...
		{
			lreach->traveltime += (unsigned short)aassettings.rs_startcrouch;
		}
		/*
		//NOTE: if there's nearby solid or a gap area after this area
...
int AAS_Reachability_Step_Barrier_WaterJump_WalkOffLedge(int area1num, int area2num) {
	...
	vector3 normal, ort, edgevec, start, end, dir;
	vector3 ground_beststart = {0, 0, 0}, ground_bestend = {0, 0, 0}, ground_bestnormal = {0, 0, 0};
	vector3 water_beststart = {0, 0, 0}, water_bestend = {0, 0, 0}, water_bestnormal = {0, 0, 0};
	vector3 invgravity = {0, 0, 1};
	vector3 testpoint;
	aas_plane_t *plane;
	aas_area_t *area1, *area2;
		...
		for (k = 0; k < groundface1->numedges; k++)
		{
			...
			for (j = 0; j < area2->numfaces; j++)
			{
				...
				for (l = 0; l < groundface2->numedges; l++)
				{
					...
					y1 = v1.z;//DotProduct(v1, invgravity) / invgravitydot;
					y2 = v2.z;//DotProduct(v2, invgravity) / invgravitydot;
					y3 = v3.z;//DotProduct(v3, invgravity) / invgravitydot;
					y4 = v4.z;//DotProduct(v4, invgravity) / invgravitydot;
					//
...
//						Log_Write("lines no overlap: from area %d to %d\r\n", area1num, area2num);
	...
	if (water_foundreach)
	{
		...
		testpoint.z -= aassettings.phys_maxwaterjump;
		...
		if (aasworld.areasettings[AAS_PointAreaNum(&testpoint)].areaflags & AREA_LIQUID)
		{
			...
			if (water_bestdist < aassettings.phys_maxwaterjump + 24)
			{
						...
						(aasworld.areasettings[area2num].presencetype & PRESENCE_NORMAL))
				{
					...
					lreach->traveltime = (unsigned short)aassettings.rs_waterjump;
					lreach->next = areareachability[area1num];
					areareachability[area1num] = lreach;
		...
		if (ground_bestdist > 0 && ground_bestdist < aassettings.phys_maxbarrier)
		{
			...
			if (!water_foundreach || (ground_bestdist - water_bestdist < 16))
			{
				...
				if (!AAS_AreaCrouch(area1num) && !AAS_AreaCrouch(area2num))
				{
					...
					lreach->traveltime = (unsigned short)aassettings.rs_barrierjump;//AAS_BarrierJumpTravelTime();
					lreach->next = areareachability[area1num];
					areareachability[area1num] = lreach;
				...
				if (!trace.startsolid && trace.fraction >= 1.0f)
				{
					...
					if (AAS_PointAreaNum(&trace.endpos) == area2num)
					{
						...
						if (i >= numareas)
						{
							...
							lreach->traveltime = (unsigned short)(aassettings.rs_startwalkoffledge + fabsf(ground_bestdist) * 50 / aassettings.phys_gravity);
							...
							if (!AAS_AreaSwim(area2num) && !AAS_AreaJumpPad(area2num))
							{
								...
								{
									lreach->traveltime += (unsigned short)aassettings.rs_falldamage5;
								}
								if (AAS_FallDelta(ground_bestdist) > aassettings.phys_falldelta10)
								{
									lreach->traveltime += (unsigned short)aassettings.rs_falldamage10;
								}
							}
							lreach->next = areareachability[area1num];
							areareachability[area1num] = lreach;
...
//	beststart1		start of the range of points on edge v1-v2
...
/*
float AAS_ClosestEdgePoints(vector3 *v1, vector3 *v2, vector3 *v3, vector3 *v4,
							aas_plane_t *plane1, aas_plane_t *plane2,
							vector3 *beststart, vector3 *bestend, float bestdist)
{
	vector3 dir1, dir2, p1, p2, p3, p4;
...
float AAS_ClosestEdgePoints(vector3 *v1, vector3 *v2, vector3 *v3, vector3 *v4,
							aas_plane_t *plane1, aas_plane_t *plane2,
							vector3 *beststart1, vector3 *bestend1,
							vector3 *beststart2, vector3 *bestend2, float bestdist)
{
	vector3 dir1, dir2, p1, p2, p3, p4;
	...
	if (dir2.x)
	{
		...
		b2 = v3->y - a2 * v3->x;
		//point on the edge vector of area2 closest to v1
		p1.x = (DotProduct(v1, &dir2) - (a2 * dir2.x + b2 * dir2.y)) / dir2.x;
		p1.y = a2 * p1.x + b2;
		//point on the edge vector of area2 closest to v2
		p2.x = (DotProduct(v2, &dir2) - (a2 * dir2.x + b2 * dir2.y)) / dir2.x;
		p2.y = a2 * p2.x+ b2;
	}
	else
	{
	...
	if (dir1.x)
	{
		...
		b1 = v1->y - a1 * v1->x;
		//point on the edge vector of area1 closest to v3
		p3.x = (DotProduct(v3, &dir1) - (a1 * dir1.x + b1 * dir1.y)) / dir1.x;
		p3.y = a1 * p3.x + b1;
		//point on the edge vector of area1 closest to v4
		p4.x = (DotProduct(v4, &dir1) - (a1 * dir1.x + b1 * dir1.y)) / dir1.x;
		p4.y = a1 * p4.x + b1;
	}
	else
	{
	...
	p4.z = 0;
	//calculate the z-coordinates from the ground planes
	p1.z = (plane2->dist - DotProduct(&plane2->normal, &p1)) / plane2->normal.z;
	p2.z = (plane2->dist - DotProduct(&plane2->normal, &p2)) / plane2->normal.z;
	p3.z = (plane1->dist - DotProduct(&plane1->normal, &p3)) / plane1->normal.z;
	p4.z = (plane1->dist - DotProduct(&plane1->normal, &p4)) / plane1->normal.z;
	//
	founddist = qfalse;
	//
	if (VectorBetweenVectors(&p1, v3, v4))
...
int AAS_Reachability_Jump(int area1num, int area2num) {
	...
	vector3 *v1, *v2, *v3, *v4;
	vector3 beststart, beststart2, bestend, bestend2;
	vector3 teststart, testend, dir, velocity, cmdmove, up = {0, 0, 1}, sidewards;
	aas_area_t *area1, *area2;
	aas_face_t *face1, *face2;
	aas_edge_t *edge1, *edge2;
	...
	for (i = 0; i < 2; i++)
	{
		if (area1->mins.data[i] > area2->maxs.data[i] + maxjumpdistance) return qfalse;
		if (area1->maxs.data[i] < area2->mins.data[i] - maxjumpdistance) return qfalse;
	}
	//if area2 is way to high to jump up to
	if (area2->mins.z > area1->maxs.z + maxjumpheight) return qfalse;
	//
	bestdist = 999999;
	//
	for (i = 0; i < area1->numfaces; i++)
...
//		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
		...
		if (bestdist <= 48 && fabs(beststart.z - bestend.z) < 8)
		{
			speed = 400;
		...
		if (trace.fraction < 1)
		{
			...
			if (DotProduct(&plane->normal, &up) >= 0.7f)
			{
				...
				{
					if (teststart.z - trace.endpos.z <= aassettings.phys_maxbarrier)
						return qfalse;
				}
			}
		}
		...
		{
			lreach->traveltime = (unsigned short)(aassettings.rs_startwalkoffledge + height * 50 / aassettings.phys_gravity);
		}
		else
		{
			lreach->traveltime = (unsigned short)(aassettings.rs_startjump + VectorDistance(&bestend, &beststart) * 240 / aassettings.phys_maxwalkvelocity);
		}
		//
		if (!AAS_AreaJumpPad(area2num))
		{
			if (AAS_FallDelta(beststart.z - bestend.z) > aassettings.phys_falldelta5)
			{
				lreach->traveltime += (unsigned short)aassettings.rs_falldamage5;
			}
			else if (AAS_FallDelta(beststart.z - bestend.z) > aassettings.phys_falldelta10)
			{
				lreach->traveltime += (unsigned short)aassettings.rs_falldamage10;
			}
		}
...
int AAS_Reachability_Ladder(int area1num, int area2num) {
	...
	vector3 area1point, area2point, v1, v2, up = {0, 0, 1};
	vector3 mid, lowestpoint = {0, 0}, start, end, sharededgevec, dir;
	aas_area_t *area1, *area2;
	...
	if (ladderface1 && ladderface2)
	{
		...
		if (ladderface1vertical)
		{
						...
						!AAS_ReachabilityExists(area2num, area1num))
			{
				...
				if (start.z - trace.endpos.z < maxjumpheight)
				{
					//create a new reachability link
					lreach = AAS_AllocReachability();
					if (!lreach) return qfalse;
...
void AAS_Reachability_Teleport( void ) {
	...
	int ent, dest;
	float angle;
	vector3 origin, destorigin, mins, maxs, end, angles;
	vector3 mid, velocity, cmdmove;
	aas_lreachability_t *lreach;
	aas_clientmove_t move;
	aas_trace_t trace;
	aas_link_t *areas, *link;
...
//#endif REACH_DEBUG
		...
		for (link = areas; link; link = link->next_area)
		{
			...
			lreach->traveltime = (unsigned short)aassettings.rs_teleport;
			lreach->next = areareachability[area1num];
			areareachability[area1num] = lreach;
			//
			reach_teleport++;
...
void AAS_Reachability_Elevator( void ) {
	...
	int ent;
	vector3 mins, maxs, origin, angles = {0, 0, 0};
	vector3 pos1, pos2, mids, platbottom, plattop;
	vector3 bottomorg, toporg, start, end, dir;
	number xvals[8], yvals[8], xvals_top[8], yvals_top[8];
	aas_lreachability_t *lreach;
	aas_trace_t trace;
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		if (!strcmp(classname, "func_plat"))
		{
			...
			if (!height) height = (maxs.z - mins.z) - lip;
			...
			//
			xvals[4] = mins.x; xvals[5] = maxs.x; xvals[6] = maxs.x; xvals[7] = mins.x;
			yvals[4] = maxs.y; yvals[5] = maxs.y; yvals[6] = mins.y; yvals[7] = mins.y;
			//find adjacent areas around the bottom of the plat
			for (i = 0; i < 9; i++)
			{
				if (i < 8) //check at the sides of the plat
				{
					bottomorg.x = origin.x + xvals[i];
					bottomorg.y = origin.y + yvals[i];
					bottomorg.z = platbottom.z + 16;
				...
				for (n = 0; n < 3; n++)
				{
					...
					xvals_top[0] = mins.x; xvals_top[1] = mids.x; xvals_top[2] = maxs.x; xvals_top[3] = mids.x;
					yvals_top[0] = mids.y; yvals_top[1] = maxs.y; yvals_top[2] = mids.y; yvals_top[3] = mins.y;
					//
					xvals_top[4] = mins.x; xvals_top[5] = maxs.x; xvals_top[6] = maxs.x; xvals_top[7] = mins.x;
					yvals_top[4] = maxs.y; yvals_top[5] = maxs.y; yvals_top[6] = mins.y; yvals_top[7] = mins.y;
					//
					for (j = 0; j < 8; j++)
					{
						toporg.x = origin.x + xvals_top[j];
						toporg.y = origin.y + yvals_top[j];
						toporg.z = plattop.z + 16;
						...
						VectorNormalize(&dir);
						dir.x = bottomorg.x + 24 * dir.x;
						dir.y = bottomorg.y + 24 * dir.y;
						dir.z = bottomorg.z;
						//
						for (p = 0; p < 3; p++) {
							if (dir.data[p] < origin.data[p] + mins.data[p] ||
								dir.data[p] > origin.data[p] + maxs.data[p] )
								break;
						}
						if (p >= 3) continue;
						//create a new reachability link
						...
						lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
						lreach->traveltime = (unsigned short)(aassettings.rs_startelevator + height * 100 / speed);
						lreach->next = areareachability[area1num];
						areareachability[area1num] = lreach;
...
aas_lreachability_t *AAS_FindFaceReachabilities(vector3 *facepoints, int numpoints, aas_plane_t *plane, int towardsface) {
	...
	vector3 beststart, beststart2, bestend, bestend2, tmp, hordir, testpoint;
	...
	for (i = 1; i < aasworld.numareas; i++)
	{
		...
		if (bestend.z - 32 > beststart.z) continue;
		//don't fall down too far
		if (bestend.z < beststart.z - 128) continue;
		...
		testpoint.z = 0;
		testpoint.z = (bestfaceplane->dist - DotProduct(&bestfaceplane->normal, &testpoint)) / bestfaceplane->normal.z;
		//
		if (!AAS_PointInsideFace(bestfacenum, &testpoint, 0.1f))
		{
			//if the faces are not overlapping then only go down
			if (bestend.z - 16 > beststart.z) continue;
		}
		lreach = AAS_AllocReachability();
...
void AAS_Reachability_FuncBobbing( void ) {
	...
	char classname[MAX_EPAIRKEY], model[MAX_EPAIRKEY];
	vector3 origin, move_end, move_start, move_start_top, move_end_top;
	vector3 mins, maxs, angles = {0, 0, 0};
	vector3 start_edgeverts[4], end_edgeverts[4], mid;
	vector3 org, start, end, dir, points[10];
	float height;
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent))
	{
		...
		//
		move_start.data[axis] -= height;
		move_end.data[axis] += height;
		//
		Log_Write("funcbob model %d, start = {%1.1f, %1.1f, %1.1f} end = {%1.1f, %1.1f, %1.1f}\n", modelnum, move_start.x, move_start.y, move_start.z, move_end.x, move_end.y, move_end.z);
		//
#ifndef BSPC
		/*
		...
		*/
		...
		for (i = 0; i < 4; i++)
		{
			...
			start_edgeverts[i].z += maxs.z - mid.z; //+ bbox maxs z
			start_edgeverts[i].z += 24;	//+ player origin to ground dist
		}
		start_edgeverts[0].x += maxs.x - mid.x;
		start_edgeverts[0].y += maxs.y - mid.y;
		start_edgeverts[1].x += maxs.x - mid.x;
		start_edgeverts[1].y += mins.y - mid.y;
		start_edgeverts[2].x += mins.x - mid.x;
		start_edgeverts[2].y += mins.y - mid.y;
		start_edgeverts[3].x += mins.x - mid.x;
		start_edgeverts[3].y += maxs.y - mid.y;
		//
		start_plane.dist = start_edgeverts[0].z;
		VectorSet(&start_plane.normal, 0, 0, 1);
		//
		for (i = 0; i < 4; i++)
		{
			VectorCopy(&move_end, &end_edgeverts[i]);
			end_edgeverts[i].z += maxs.z - mid.z; //+ bbox maxs z
			end_edgeverts[i].z += 24;	//+ player origin to ground dist
		}
		end_edgeverts[0].x += maxs.x - mid.x;
		end_edgeverts[0].y += maxs.y - mid.y;
		end_edgeverts[1].x += maxs.x - mid.x;
		end_edgeverts[1].y += mins.y - mid.y;
		end_edgeverts[2].x += mins.x - mid.x;
		end_edgeverts[2].y += mins.y - mid.y;
		end_edgeverts[3].x += mins.x - mid.x;
		end_edgeverts[3].y += maxs.y - mid.y;
		//
		end_plane.dist = end_edgeverts[0].z;
		...
		move_start_top.z += maxs.z - mid.z + 24; //+ bbox maxs z
		VectorCopy(&move_end, &move_end_top);
		move_end_top.z += maxs.z - mid.z + 24; //+ bbox maxs z
		//
		...
		for (i = 0; i < 2; i++)
		{
			...
			for (startreach = firststartreach; startreach; startreach = nextstartreach)
			{
				...
				for (endreach = firstendreach; endreach; endreach = nextendreach)
				{
					...
					if (i == 0) lreach->edgenum = ((int)move_start.data[axis] << 16) | ((int) move_end.data[axis] & 0x0000ffff);
					else lreach->edgenum = ((int)move_end.data[axis] << 16) | ((int) move_start.data[axis] & 0x0000ffff);
...
//					AAS_PermanentLine(lreach->start, lreach->end, 1);
					...
					lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
					lreach->traveltime = (unsigned short)aassettings.rs_funcbob;
					reach_funcbob++;
...
void AAS_Reachability_JumpPad( void ) {
	...
	aas_area_t *area2;
	aas_lreachability_t *lreach;
	vector3 areastart, facecenter, dir, cmdmove;
	vector3 velocity, absmins, absmaxs;
	//vector3 origin, ent2origin, angles, teststart;
	aas_clientmove_t move;
	//aas_trace_t trace;
	aas_link_t *areas, *link;
...
//		VectorScale(velocity, speed, velocity);
        ...
        */
		...
		if (velocity.x || velocity.y)
		{
			...
			if (area2num && i < 20)
			{
				for (link = areas; link; link = link->next_area)
				{
					...
					lreach->facenum = (int)velocity.z;
					//NOTE: the edgenum is the horizontal velocity
					lreach->edgenum = (int)sqrtf(velocity.x*velocity.x + velocity.y*velocity.y);
					VectorCopy(&areastart, &lreach->start);
					VectorCopy(&move.endpos, &lreach->end);
					lreach->traveltype = TRAVEL_JUMPPAD;
					lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
					lreach->traveltime = (unsigned short)aassettings.rs_jumppad;
		...
		//
		if (fabsf(velocity.x) > 100 || fabsf(velocity.y) > 100) continue;
		...
		for (area2num = 1; area2num < aasworld.numareas; area2num++)
		{
			...
			for (i = 0; i < area2->numfaces; i++)
			{
				...
				//only go higher up
				if (facecenter.z < areastart.z) continue;
								...
								&& (move.stopevent & (SE_HITGROUNDAREA|SE_TOUCHJUMPPAD|SE_TOUCHTELEPORTER)))
						{
							...
							if (!link)
							{
								for (link = areas; link; link = link->next_area)
								{
									...
									lreach->facenum = (int)velocity.z;
									//NOTE: the edgenum is the horizontal velocity
									lreach->edgenum = (int)sqrtf(cmdmove.x*cmdmove.x + cmdmove.y*cmdmove.y);
									VectorCopy(&areastart, &lreach->start);
									VectorCopy(&facecenter, &lreach->end);
									lreach->traveltype = TRAVEL_JUMPPAD;
									lreach->traveltype |= AAS_TravelFlagsForTeam(ent);
									lreach->traveltime = (unsigned short)aassettings.rs_aircontrolledjumppad;
									lreach->next = areareachability[link->areanum];
									areareachability[link->areanum] = lreach;
									//
									reach_jumppad++;
...
int AAS_Reachability_Grapple(int area1num, int area2num) {
	...
	aas_area_t *area1, *area2;
	aas_lreachability_t *lreach;
	vector3 areastart, facecenter, start, end, dir, down = {0, 0, -1};
	vector3 *v;
	...
	for (i = 0; i < area2->numfaces; i++)
	{
		...
		if (facecenter.z < areastart.z + 64) continue;
		...
		lreach->traveltime = (unsigned short)(aassettings.rs_startgrapple + VectorLength(&dir) * 0.25f);
		lreach->next = areareachability[area1num];
		areareachability[area1num] = lreach;
		//
		reach_grapple++;
...
int AAS_Reachability_WeaponJump(int area1num, int area2num) {
	...
	aas_area_t *area1, *area2;
	aas_lreachability_t *lreach;
	vector3 areastart, facecenter, start, end, dir, cmdmove;// teststart;
	vector3 velocity;
	aas_clientmove_t move;
	aas_trace_t trace;
					...
					*/
								...
								&& (move.stopevent & (SE_HITGROUNDAREA|SE_TOUCHJUMPPAD)))
					{
						...
						if (n)
						{
							...
							lreach->traveltime = (unsigned short)aassettings.rs_bfgjump;
						}
						else
						{
							lreach->traveltype = TRAVEL_ROCKETJUMP;
							lreach->traveltime = (unsigned short)aassettings.rs_rocketjump;
						}
						lreach->next = areareachability[area1num];
						areareachability[area1num] = lreach;
						//
...
void AAS_Reachability_WalkOffLedge(int areanum) {
	...
	aas_plane_t *plane;
	vector3 *v1, *v2;
	vector3 sharededgevec, mid, dir, testend;
	aas_lreachability_t *lreach;
	aas_trace_t trace;
			...
			for (j = 0; j < area->numfaces; j++)
			{
				...
				for (l = 0; l < face2->numedges; l++)
				{
					...
					if (abs(edge1num) == abs(edge2num))
					{
						...
						if (aassettings.rs_maxfallheight && fabs(mid.z - trace.endpos.z) > aassettings.rs_maxfallheight)
							break;
						//
						lreach = AAS_AllocReachability();
						if (!lreach) break;
						...
						VectorCopy(&trace.endpos, &lreach->end);
						lreach->traveltype = TRAVEL_WALKOFFLEDGE;
						lreach->traveltime = (unsigned short)(aassettings.rs_startwalkoffledge + fabs(mid.z - trace.endpos.z) * 50 / aassettings.phys_gravity);
						if (!AAS_AreaSwim(reachareanum) && !AAS_AreaJumpPad(reachareanum))
						{
								 if (AAS_FallDelta(mid.z - trace.endpos.z) > aassettings.phys_falldelta5)
								lreach->traveltime += (unsigned short)aassettings.rs_falldamage5;
							else if (AAS_FallDelta(mid.z - trace.endpos.z) > aassettings.phys_falldelta10)
								lreach->traveltime += (unsigned short)aassettings.rs_falldamage10;
						}
...
#ifndef BSPC
	calcgrapplereach = (int)LibVarGetValue("grapplereach");
#endif
