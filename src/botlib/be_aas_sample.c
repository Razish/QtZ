...
#define ON_EPSILON					0 //0.0005f

#define TRACEPLANE_EPSILON			0.125f

typedef struct aas_tracestack_s
{
	vector3 start;		//start point of the piece of line to trace
	vector3 end;			//end point of the piece of line to trace
...
int AAS_PointReachabilityAreaIndex( vector3 *origin ) {
...
// calculates the minimum distance between the origin of the box and the given plane when both will collide
//	on the given side of the plane
...
number AAS_BoxOriginDistanceFromPlane(vector3 *normal, vector3 *mins, vector3 *maxs, int side) {
	vector3 v1, v2;
	int i;
	...
	if (side)
	{
		...
		for ( i=0; i<3; i++ ) {
				 if ( normal->data[i] >  BBOX_NORMAL_EPSILON )	v1.data[i] = maxs->data[i];
			else if ( normal->data[i] < -BBOX_NORMAL_EPSILON )	v1.data[i] = mins->data[i];
			else												v1.data[i] = 0;
		}
	}
	else
	{
		...
		//to collide with the plane
		for ( i=0; i<3; i++ ) {
				 if ( normal->data[i] >  BBOX_NORMAL_EPSILON )	v1.data[i] = mins->data[i];
			else if ( normal->data[i] < -BBOX_NORMAL_EPSILON )	v1.data[i] = maxs->data[i];
			else												v1.data[i] = 0;
		}
	}
	//
...
qboolean AAS_AreaEntityCollision(int areanum, vector3 *start, vector3 *end, int presencetype, int passent, aas_trace_t *trace) {
	int collision;
	...
	while (1)
	{
		...
		if (nodenum < 0)
		{
			...
			if (!(aasworld.areasettings[-nodenum].presencetype & presencetype))
			{
				...
				if ( VectorCompare( &tstack_p->start, start ) ) {
...
//			VectorSubtract(end, start, v1);
		...
		else
		{
			...
			//
			cur_mid.x = cur_start.x + (cur_end.x - cur_start.x) * frac;
			cur_mid.y = cur_start.y + (cur_end.y - cur_start.y) * frac;
			cur_mid.z = cur_start.z + (cur_end.z - cur_start.z) * frac;
...
int AAS_TraceAreas(vector3 *start, vector3 *end, int *areas, vector3 *points, int maxareas) {
	...
	int numareas;
	float front, back, frac;
	vector3 cur_start, cur_end, cur_mid;
	aas_tracestack_t tracestack[127];
	aas_tracestack_t *tstack_p;
	aas_node_t *aasnode;
	aas_plane_t *plane;
	...
	while (1)
	{
		...
		else
		{
			...
			//frac = front / (front-back);
			//
			VectorLerp( &cur_start, frac, &cur_end, &cur_mid );
...
qboolean AAS_InsideFace(aas_face_t *face, vector3 *pnormal, vector3 *point, float epsilon) {
	...
	vector3 v0;
	vector3 edgevec, pointvec, sepnormal;
	aas_edge_t *edge;
#ifdef AAS_SAMPLE_DEBUG
	int lastvertex = 0;
#endif //AAS_SAMPLE_DEBUG
...
aas_face_t *AAS_AreaGroundFace(int areanum, vector3 *point) {
	...
	for (i = 0; i < area->numfaces; i++)
	{
		...
		if (face->faceflags & FACE_GROUND)
		{
			...
			if (aasworld.planes[face->planenum].normal.z < 0) VectorNegate(&up, &normal);
...
int AAS_BoxOnPlaneSide2(vector3 *absmins, vector3 *absmaxs, aas_plane_t *p) {
	int i, sides;
	float dist1, dist2;
	vector3 corners[2];
	...
	for (i = 0; i < 3; i++)
	{
		if (p->normal.data[i] < 0)
		{
			corners[0].data[i] = absmins->data[i];
			corners[1].data[i] = absmaxs->data[i];
		}
		else
		{
			corners[1].data[i] = absmins->data[i];
			corners[0].data[i] = absmaxs->data[i];
		}
	}
...
int AAS_BoxOnPlaneSide( vector3 *absmins, vector3 *absmaxs, aas_plane_t *p ) {
	if ( p->type < 3)
	{
		if ( p->dist <= absmins->data[p->type] )
			return 1;
		else {
			if ( p->dist >= absmaxs->data[p->type] )
				return 2;
			else
				return 3;
		}
...
// link the entity to the areas the bounding box is totally or partly situated in. This is done with recursion
//	down the tree using the bounding box to test for plane sides
