...
#if 0
static const char *shaderSourceBlur[2] = {
...
{\n\
	...
	vec3 color = vec3( 0.0 );\n\
	color += 1.0  * texture2D( SceneTex, coord + vec2( -4.0 * (PixelWidth), 0.0 ) ).rgb;\n\
	color += 8.0  * texture2D( SceneTex, coord + vec2( -3.0 * (PixelWidth), 0.0 ) ).rgb;\n\
	color += 28.0 * texture2D( SceneTex, coord + vec2( -2.0 * (PixelWidth), 0.0 ) ).rgb;\n\
	color += 56.0 * texture2D( SceneTex, coord + vec2( -1.0 * (PixelWidth), 0.0 ) ).rgb;\n\
	color += 70.0 * texture2D( SceneTex, coord ).rgb;\n\
	color += 56.0 * texture2D( SceneTex, coord + vec2( 1.0 * (PixelWidth), 0.0 ) ).rgb;\n\
	color += 28.0 * texture2D( SceneTex, coord + vec2( 2.0 * (PixelWidth), 0.0 ) ).rgb;\n\
	color += 8.0  * texture2D( SceneTex, coord + vec2( 3.0 * (PixelWidth), 0.0 ) ).rgb;\n\
	color += 1.0  * texture2D( SceneTex, coord + vec2( 4.0 * (PixelWidth), 0.0 ) ).rgb;\n\
...
{\n\
	vec2 coord = gl_TexCoord[0].st;\n\
	vec3 color = vec3( 0.0 );\n\
	color += 1.0  * texture2D( SceneTex, coord + vec2( 0.0, -4.0 * (PixelHeight) ) ).rgb;\n\
	color += 8.0  * texture2D( SceneTex, coord + vec2( 0.0, -3.0 * (PixelHeight) ) ).rgb;\n\
	color += 28.0 * texture2D( SceneTex, coord + vec2( 0.0, -2.0 * (PixelHeight) ) ).rgb;\n\
	color += 56.0 * texture2D( SceneTex, coord + vec2( 0.0, -1.0 * (PixelHeight) ) ).rgb;\n\
	color += 70.0 * texture2D( SceneTex, coord ).rgb;\n\
	color += 56.0 * texture2D( SceneTex, coord + vec2( 0.0, 1.0 * (PixelHeight) ) ).rgb;\n\
	color += 28.0 * texture2D( SceneTex, coord + vec2( 0.0, 2.0 * (PixelHeight) ) ).rgb;\n\
	color += 8.0  * texture2D( SceneTex, coord + vec2( 0.0, 3.0 * (PixelHeight) ) ).rgb;\n\
...
{
	float	w = glConfig.vidWidth, h = glConfig.vidHeight;
	float	ratio = h/w;
	float	size = LUMINANCE_FBO_SIZE;
	int i;
	int steps = r_postprocess_bloomSteps->integer;
	if ( steps < 1 ) steps = 1;
	if ( steps > NUM_BLOOM_DOWNSCALE_FBOS ) steps = NUM_BLOOM_DOWNSCALE_FBOS;
	...
	//scene
	tr.framebuffers.scene					= R_EXT_CreateFBO( w, h, IF_RGBA16F, IF_DEPTH24_STENCIL8 );
//	R_EXT_AttachDepthStencilTextureToFramebuffer( tr.framebuffers.scene, R_EXT_CreateTexture( w, h, IF_DEPTH24_STENCIL8, 1, 1 ) );

	//dynamic glow
	tr.framebuffers.glow					= R_EXT_CreateFBO( w, h, IF_RGBA16F, IF_DEPTH24_STENCIL8 );
...
//	tr.framebuffers.glow->stencilTexture = tr.framebuffers.scene->stencilTexture;
	...
	for ( i=0, size=1.0f; i<NUM_GLOW_DOWNSCALE_FBOS; i++, size *= GLOW_DOWNSCALE_RATE )
		tr.framebuffers.glowDownscale[i]	= R_EXT_CreateFBO( floorf( tr.postprocessing.glowSize/size ), floorf( tr.postprocessing.glowSize/size ), IF_RGBA8, 0 );
	for ( i=0; i<2; i++ )
		tr.framebuffers.glowBlur[i]			= R_EXT_CreateFBO( floorf( tr.postprocessing.glowSize/size ), floorf( tr.postprocessing.glowSize/size ), IF_RGBA8, 0 );
	tr.framebuffers.glowComposite			= R_EXT_CreateFBO( w, h, IF_RGBA8, 0 );
	...
	//final pass
	tr.framebuffers.final					= R_EXT_CreateFBO( w, h, IF_RGBA8, 0 );
}
...
static void CalculateLightAdaptation ( void )
{
	...
	float	dt;
	vec4_t	currentFrameLuminance = { 0.0f };
	float tau = 1.0f;
	...
	tau = (currentFrameLuminance[0] * r_postprocess_hdrLightAdaptRods->value) + ((1.0-currentFrameLuminance[0]) * r_postprocess_hdrLightAdaptCones->value);
	...
	adaptedFrameLuminance	+= (currentFrameLuminance[0] - adaptedFrameLuminance)	* (1.0f - exp(-dt * tau));
	adaptedMaxLuminance		+= (currentFrameLuminance[1] - adaptedMaxLuminance)		* (1.0f - exp(-dt * tau));
	lastFrameTime = time;
}

static __inline void ResizeTarget( float w, float h )
{
	glViewport( 0, 0, w, h );
	glScissor( 0, 0, w, h );
}
...
//	R_EXT_BlitFramebufferColorAndDepth( NULL, tr.framebuffers.scene, w, h, w, h );
...
#if 1 //Disabling dynamic glow for now, there is a fatal problem with depth testing - glow surfaces are never obstructed by other geometry :/
	...
	if ( r_postprocess_hdr->integer )
	{
		...
		adaptedMaxLuminance = max( adaptedMaxLuminance, 0.45 );
		adaptedMaxLuminance = min( adaptedMaxLuminance, 0.9 );
		adaptedFrameLuminance = max( adaptedFrameLuminance, 0.18 );

			 if ( r_postprocess_hdrKeyStyle->integer == 1 )
			imageKey = 0.75 / (r_postprocess_brightPassThres->value + log10( adaptedMaxLuminance ));
		else if ( r_postprocess_hdrKeyStyle->integer == 2 )
			imageKey = /*1.03 -*/ ( 2.0 / (2.0 + log10f( adaptedFrameLuminance )) );
		else if ( r_postprocess_hdrKeyStyle->integer == 3 )
			imageKey = /*1.03 -*/ ( 2.0 / (2.0 + log10f( adaptedMaxLuminance )) );
		else if ( r_postprocess_hdrKeyStyle->integer == 4 )
			imageKey = /*1.03 -*/ ( 1.0 / (1.0 + log10f( adaptedFrameLuminance )) );
		else if ( r_postprocess_hdrKeyStyle->integer == 5 )
			imageKey = /*1.03 -*/ ( 1.0 / (1.0 + log10f( adaptedMaxLuminance )) );

		R_EXT_BindFramebuffer( tr.framebuffers.tonemapping );
		R_EXT_GLSL_UseProgram( tr.glsl.tonemapping );
		R_EXT_GLSL_SetUniform1f( tr.glsl.tonemapping, "ImageKey", imageKey );
		R_EXT_GLSL_SetUniform1f( tr.glsl.tonemapping, "Luminance", adaptedFrameLuminance );
		R_EXT_GLSL_SetUniform1f( tr.glsl.tonemapping, "MaxWhite", adaptedMaxLuminance*adaptedMaxLuminance );
		R_EXT_GLSL_SetUniform1i( tr.glsl.tonemapping, "Operator", r_postprocess_hdrOperator->integer );
		GL_Bind( lastFullscreenFBO->colorTextures[0]->q3image );
        DrawFullscreenQuad();
	...
	if ( r_postprocess_bloom->integer )
	{
		...
		R_EXT_GLSL_SetUniform1f( tr.glsl.brightPass, "Threshold", r_postprocess_brightPassThres->value );
		R_EXT_GLSL_SetUniform1i( tr.glsl.brightPass, "Style", r_postprocess_brightPassStyle->integer );
		GL_Bind( lastFullscreenFBO->colorTextures[0]->q3image );
		DrawFullscreenQuad();
		...
		R_EXT_GLSL_SetUniform1f( tr.glsl.bloom, "BloomMulti", r_postprocess_bloomMulti->value );
		R_EXT_GLSL_SetUniform1f( tr.glsl.bloom, "BloomSat", r_postprocess_bloomSat->value );
		R_EXT_GLSL_SetUniform1f( tr.glsl.bloom, "SceneMulti", r_postprocess_bloomSceneMulti->value );
		R_EXT_GLSL_SetUniform1f( tr.glsl.bloom, "SceneSat", r_postprocess_bloomSceneSat->value );
		R_EXT_GLSL_SetUniform1i( tr.glsl.bloom, "BloomStyle", r_postprocess_bloomStyle->integer );
		DrawFullscreenQuad();
	...
	if ( r_postprocess_finalPass->integer )
	{
		...
		R_EXT_GLSL_SetUniform4f( tr.glsl.final, "ColourGrading", tr.postprocessing.grading[0], tr.postprocessing.grading[1], tr.postprocessing.grading[2], tr.postprocessing.grading[3] );
		if ( sscanf( r_postprocess_userVec1->string, "%f %f %f %f", &userVec1[0], &userVec1[1], &userVec1[2], &userVec1[3] ) == 4 )
			R_EXT_GLSL_SetUniform4f( tr.glsl.final, "UserVec1", userVec1[0], userVec1[1], userVec1[2], userVec1[3] );
		else
			R_EXT_GLSL_SetUniform4f( tr.glsl.final, "UserVec1", tr.postprocessing.grading[0], tr.postprocessing.grading[1], tr.postprocessing.grading[2], tr.postprocessing.grading[3] );
		GL_Bind( lastFullscreenFBO->colorTextures[0]->q3image );
		DrawFullscreenQuad();
