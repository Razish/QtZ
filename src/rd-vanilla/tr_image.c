...
static void Upload32( unsigned *data, int width, int height, qboolean mipmap, qboolean picmip, qboolean lightMap, int *format, int *pUploadWidth, int *pUploadHeight, qboolean noTC )
{
	...
	if( r_greyscale->integer )
	{
		for ( i = 0; i < c; i++ )
		{
			byte luma = (byte)LUMA(scan[i*4], scan[i*4 + 1], scan[i*4 + 2]);
			scan[i*4] = luma;
			scan[i*4 + 1] = luma;
			scan[i*4 + 2] = luma;
		}
	...
	else if( r_greyscale->value )
	{
		for ( i = 0; i < c; i++ )
		{
			...
			scan[i*4] = (byte)LERP(scan[i*4], luma, r_greyscale->value);
			scan[i*4 + 1] = (byte)LERP(scan[i*4 + 1], luma, r_greyscale->value);
			scan[i*4 + 2] = (byte)LERP(scan[i*4 + 2], luma, r_greyscale->value);
		}
	}
	...
	{// developers will see rainbow textures
		...
		for ( x=0; x<DEVELOPER_SIZE; x++ )
		{
			for ( y=0; y<DEVELOPER_SIZE; y++ )
			{
				...
				data.developer[x][y][0] = (byte)(r*255.0f);
				data.developer[x][y][1] = (byte)(g*255.0f);
				data.developer[x][y][2] = (byte)(b*255.0f);
